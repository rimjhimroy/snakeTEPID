rule mergemapped:
    ...
    input:
        ...
        ufqin = lambda wildcards: expand("output/align/{samples}/{samples}.umap.fastq",samples=args['LANEDICT'][wildcards.individual])

    output:
        bamout= "output/merged/{individual}/{individual}.bam",
        splitout= "output/merged/{individual}/{individual}.split.bam",
        ufqout= "output/merged/{individual}/{individual}.umap.fastq",
        baiout="output/merged/{individual}/{individual}.bai"
    log:  
